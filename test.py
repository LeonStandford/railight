...
from utils.metrics import detect_metrics_from_cm, embedding_overlap_stats
...
def evaluate(args_ns: argparse.Namespace) -> Dict[str, float]:
    ...
    per_image, all_img_paths, elapsed, infer_s, n_imgs_timed = _run_source_pass(
        net, val_loader, use_cuda, fp_fn_writer
    )
    ...
    _align_bank_src: List[torch.Tensor] = []
    _align_bank_tgt: List[torch.Tensor] = []

    def _collect(imgs, feat_acc, fbank, pbank, before_acc, before_bank, align_bank):
        emb = net.embed_features(imgs)
        fbank.append(emb.detach().float().cpu())
        if hasattr(net, "embed_align_features"):
            align_bank.append(net.embed_align_features(imgs).detach().float().cpu())
        before_bank.append(
    ...
    try:
            ...
            with torch.no_grad():
                for s_imgs, _t, _p in tqdm(
                    ...
                    _collect(
                        ...
                        feat_before_day, _src_before_bank, _align_bank_src,
                    )
            ...
            with torch.no_grad():
                for t_imgs, _t, _p in tqdm(
                    ...
                    _collect(
                        ...
                        feat_before_night, _tgt_before_bank, _align_bank_tgt,
                    )
        elif run_dist_pass:
            print(
        ...
        if run_domain_gap and _src_bank and _tgt_bank:
        ...
        if run_domain_gap and _src_prob and _tgt_prob:
    ...
    dist_stats.update(
        embedding_overlap_stats(_align_bank_src, _align_bank_tgt)
    )
    source_eval = evaluate_domain(
        "source", per_image, float(args_ns.iou_thr),
        float(args_ns.score_thr_cm), int(args_ns.nc),
    )
    ...
    try:
        tsne_panels = []
        if run_tsne and _src_before_bank and _tgt_before_bank:
            tsne_panels.append((
                "raw input pixels (illumination-variant)",
                torch.cat(_src_before_bank, dim=0).numpy(),
                torch.cat(_tgt_before_bank, dim=0).numpy(),
            ))
        if run_tsne and _src_bank and _tgt_bank:
            tsne_panels.append((
                "g_f from reflectance encoder (shallow tap)",
                torch.cat(_src_bank, dim=0).numpy(),
                torch.cat(_tgt_bank, dim=0).numpy(),
            ))
        if run_tsne and _align_bank_src and _align_bank_tgt:
            tsne_panels.append((
                "whitened multi-tap embedding (the aligned space)",
                torch.cat(_align_bank_src, dim=0).numpy(),
                torch.cat(_align_bank_tgt, dim=0).numpy(),
            ))
        if tsne_panels:
            viz.plot_domain_tsne_pair(
                tsne_panels, charts_dir, "tsne_reflectance.png",
                method=method, config=config_dict,
                suptitle=(
                    "tSNE of illumination invariant representation between "
                    "source and target domain from reflectance encoder of "
                    "VGG16 backbone"
                ),
            )
    except Exception as e:
