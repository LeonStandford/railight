...
def evaluate(args_ns: argparse.Namespace) -> Dict[str, float]:
    ...
    export_dir = getattr(args_ns, "export_predicted_target_path", None)
    if export_dir:
        n = _export_yolo_predictions(
            per_image, all_img_paths, str(export_dir), int(args_ns.nc)
        )
        print(f"[export] wrote {n} YOLO prediction files to {export_dir}")

    feat_day: List[float] = []         # AFTER extract_features (pooled conv4_3)
    feat_night: List[float] = []       # AFTER extract_features (pooled conv4_3)
    feat_before_day: List[float] = []  # BEFORE extract_features (raw input), day
    feat_before_night: List[float] = []  # BEFORE extract_features, night
    kl_vals: List[float] = []
    ce_vals: List[float] = []          # cross-entropy day<->night (single)
    _src_bank: List[torch.Tensor] = []   # pooled feats AFTER extract (KL+t-SNE)
    _tgt_bank: List[torch.Tensor] = []
    _src_before_bank: List[torch.Tensor] = []  # raw-input vec BEFORE extract
    _tgt_before_bank: List[torch.Tensor] = []
    _src_prob: List[torch.Tensor] = []   # mean softmax class dist / image
    _tgt_prob: List[torch.Tensor] = []

    def _collect(imgs, feat_acc, fbank, pbank, before_acc, before_bank):
        emb = net.embed_features(imgs)  # (B, C) pooled conv4_3
        fbank.append(emb.detach().float().cpu())
        # BEFORE extract_features: cheap pooled raw-input embedding (B, 3*8*8)
        before_bank.append(
        ...
        # before_acc = raw input per-image mean activation (BEFORE the DSFD
        #              backbone feature extractor) for this domain
        # feat_acc   = pooled conv4_3 per-image mean activation (AFTER it)
        before_acc.extend(
        ...
        c = o[4]                        # (B, num_priors, num_classes)
        # mean predicted class distribution per image (over anchors)
        pbank.append(
    ...
    try:
        ...
        # Cross-entropy distribution BETWEEN day & night predicted class
        # distributions: H(p_day, p_night) = -sum p_day * log p_night,
        # sampled over random day/night image pairs (single distribution).
        ...
        if use_cuda:
            torch.set_default_tensor_type(  # noqa: deprecated-ok
    ...
    # Plot the FULL (nc+1)x(nc+1) matrix incl. the background row/col.
    # Slicing to cm[:nc,:nc] drops every FN (class -> background, missed)
    # and FP (background -> class, spurious), which made the matrix a
    # trivial 1.00 diagonal that contradicted accuracy/precision/recall.
    ...
    # Scatter (t-SNE) source vs target, BEFORE / AFTER DSFD.extract_features
    # -> two separate files (not merged into test_distributions.png).
    try:
    ...
    # Grad-CAM: source (day) vs target (night) -> its own image.
    try:
        ...
        if conv_layers:
            ...
            # Target (night): accumulate n_show across batches.
            tgt_items: list = []
            tgt_dir = getattr(args_ns, "target_folder", "")
            if tgt_dir and os.path.isdir(tgt_dir):
    ...
    # Qualitative only: predictions on real night target frames
    # (no labels -> not used for metrics, just visual sanity check).
    ...
    # Persist metrics to records/<arch>/<backbone>/<num_exp>_test.csv
            ...
            for i in range(images.shape[0]):
                if len(day) < n_show:
                    ...
                    day.append(
                        dict(
                            image=(
                                images[i].detach().cpu().numpy().transpose(
                                    1, 2, 0
                                )[:, :, ::-1]
                                * 255
                            )
                ...
                if len(synth) < n_show:
                    pb_n, ps_n, pl_n = infer_detections(
                        net, img_dark[i], conf_thr=0.5
                    )
                    synth.append(
                        dict(
                            image=(
                                img_dark[i].detach().cpu().numpy().transpose(
                                    1, 2, 0
                                )[:, :, ::-1]
                                * 255
                            )
