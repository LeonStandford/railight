...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--max-class", default=3, type=int, help="Drop label rows with cls >= max_class"
    )
    p.add_argument(
        "--cls-offset",
        default=1,
...
def _yolo_line_to_pixel(
    line: str, w: int, h: int, max_class: int, cls_offset: int
...
def convert_split(
    images_dir: str, labels_dir: str, out_path: str, max_class: int, cls_offset: int
) -> int:
    ...
    with open(out_path, "w") as out:
        for name in tqdm(
            files, desc=os.path.basename(out_path), colour="red", dynamic_ncols=True
        ):
            ...
            with open(label_path) as fh:
                for line in fh:
                    parsed = _yolo_line_to_pixel(line, w, h, max_class, cls_offset)
