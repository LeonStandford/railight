...
class VideoFrameCutter:
    ...
    def cut(
        self, video_path: str, out_dir: str, outer_bar: Optional[tqdm] = None
    ) -> int:
        ...
        step = max(1, int(round(fps_src / max(cfg.fps_target, 1e-06))))
        ...
        try:
            while True:
                ...
                if idx % step == 0:
                    ...
                    cv2.imwrite(os.path.join(out_dir, fname), frame, write_params)
                    saved += 1
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--path-input", required=True, type=str, help="Folder containing source videos."
    )
    p.add_argument(
        "--path-output",
        required=True,
    ...
    p.add_argument(
        "--fps", type=float, default=1.0, help="Target sampling rate (frames/second)."
    )
    p.add_argument(
...
def main() -> None:
    ...
    if not os.path.isdir(args.path_input):
        raise FileNotFoundError(f"path-input does not exist: {args.path_input}")
    os.makedirs(args.path_output, exist_ok=True)
    videos = _list_videos(args.path_input)
    if not videos:
        print(f"No videos with extensions {VIDEO_EXTS} found in {args.path_input}")
        return
    print(
        f"Found {len(videos)} videos. Sampling at ~{args.fps} fps -> {args.path_output}"
    )
    cutter = VideoFrameCutter(
        FrameCutterConfig(
            fps_target=args.fps,
