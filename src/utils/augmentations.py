...
def random_saturation(img):
    ...
    if prob < cfg.saturation_prob:
        delta = np.random.uniform(-cfg.saturation_delta, cfg.saturation_delta) + 1
...
def meet_emit_constraint(src_bbox, sample_bbox):
    ...
    if (
        center_x >= sample_bbox.xmin
        and center_x <= sample_bbox.xmax
        and (center_y >= sample_bbox.ymin)
        and (center_y <= sample_bbox.ymax)
    ):
        return True
    return False
...
def project_bbox(object_bbox, sample_bbox):
    if (
        object_bbox.xmin >= sample_bbox.xmax
        or object_bbox.xmax <= sample_bbox.xmin
        or object_bbox.ymin >= sample_bbox.ymax
        or (object_bbox.ymax <= sample_bbox.ymin)
    ):
        return False
    else:
...
def transform_labels(bbox_labels, sample_bbox):
    ...
    for i in range(len(bbox_labels)):
        sample_label = []
        object_bbox = bbox(
            bbox_labels[i][1], bbox_labels[i][2], bbox_labels[i][3], bbox_labels[i][4]
        )
...
def intersect_bbox(bbox1, bbox2):
    if (
        ...
        or bbox2.xmax < bbox1.xmin
        or bbox2.ymin > bbox1.ymax
        or (bbox2.ymax < bbox1.ymin)
    ):
        ...
        intersection_box = bbox(
            ...
            min(bbox1.xmax, bbox2.xmax),
            min(bbox1.ymax, bbox2.ymax),
        )
...
def jaccard_overlap(sample_bbox, object_bbox):
    if (
        sample_bbox.xmin >= object_bbox.xmax
        or sample_bbox.xmax <= object_bbox.xmin
        or sample_bbox.ymin >= object_bbox.ymax
        or (sample_bbox.ymax <= object_bbox.ymin)
    ):
        return 0
...
def satisfy_sample_constraint(sampler, sample_bbox, bbox_labels):
    ...
    for i in range(len(bbox_labels)):
        ...
        if has_jaccard_overlap:
            ...
            if (
                sampler.min_jaccard_overlap != 0
                and overlap < sampler.min_jaccard_overlap
            ):
                continue
            if (
                sampler.max_jaccard_overlap != 0
                and overlap > sampler.max_jaccard_overlap
            ):
                continue
            found = True
        if has_object_coverage:
            ...
            if (
                sampler.min_object_coverage != 0
                and object_coverage < sampler.min_object_coverage
            ):
                continue
            if (
                sampler.max_object_coverage != 0
                and object_coverage > sampler.max_object_coverage
            ):
                continue
            found = True
        if found:
...
def crop_image_sampling(
    ...
    sample_labels = transform_labels_sampling(
        bbox_labels, sample_bbox, resize_val, min_face_size
    )
    return (sample_img, sample_labels)


def transform_labels_sampling(bbox_labels, sample_bbox, resize_val, min_face_size):
    sample_labels = []
    for i in range(len(bbox_labels)):
        sample_label = []
        object_bbox = bbox(
...
def anchor_crop_image_sampling(img, bbox_labels, scale_array, img_width, img_height):
    ...
    _anchor_choices = scale_array[0 : min(anchor_idx + 1, 5) + 1]
    target_anchor = _anchor_choices[random.randrange(len(_anchor_choices))]
    ...
    image = cv2.resize(img, None, None, fx=ratio, fy=ratio, interpolation=interp_method)
    boxes[:, 0] *= ratio
    boxes[:, 1] *= ratio
    boxes[:, 2] *= ratio
    boxes[:, 3] *= ratio
    ...
    if len(sample_boxes) > 0:
        ...
        if choice_box[0] < 0 or choice_box[1] < 0:
            ...
            current_image = image_pad[
                start_top:end_bottom, start_left:end_right, :
            ].copy()
        ...
        current_image = image[
            choice_box[1] : choice_box[3], choice_box[0] : choice_box[2], :
        ].copy()
        image_height, image_width, _ = current_image.shape
...
def anchor_crop_image_sampling2(
    ...
    interp_method = random.choice(interp_methods)
    image = cv2.resize(img, None, None, fx=ratio, fy=ratio, interpolation=interp_method)
    dark_image = cv2.resize(
        dark_img, None, None, fx=ratio, fy=ratio, interpolation=interp_method
    )
    boxes[:, 0] *= ratio
    boxes[:, 1] *= ratio
    boxes[:, 2] *= ratio
    ...
    if len(sample_boxes) > 0:
        ...
        if choice_box[0] < 0 or choice_box[1] < 0:
            ...
            image_pad = np.zeros((new_img_height, new_img_width, 3), dtype=float)
            image_pad[:, :, :] = mean
            darkimage_pad = np.zeros((new_img_height, new_img_width, 3), dtype=float)
            darkimage_pad[:, :, :] = mean
            start_left = 0 if choice_box[0] >= 0 else -choice_box[0]
            start_top = 0 if choice_box[1] >= 0 else -choice_box[1]
            image_pad[start_top:, start_left:, :] = image
...
class StrongAugmentor:
    ...
    def describe(self) -> str:
        return "color/RGB-shift p=0.8 -> blur/noise p=0.4 -> shadow p=0.2"
