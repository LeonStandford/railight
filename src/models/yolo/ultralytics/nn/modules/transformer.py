...
class TransformerEncoderLayer(nn.Module):
    ...
    @staticmethod
    def with_pos_embed(
        tensor: torch.Tensor, pos: torch.Tensor | None = None
    ) -> torch.Tensor:
...
class AIFI(TransformerEncoderLayer):
    ...
    def build_2d_sincos_position_embedding(
        ...
        grid_w, grid_h = (
            torch.meshgrid(grid_w, grid_h, indexing="ij")
            if TORCH_1_11
            else torch.meshgrid(grid_w, grid_h)
        )
        pos_dim = embed_dim // 4
...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, layer in enumerate(self.layers):
            x = (
                getattr(self, "act", nn.ReLU())(layer(x))
