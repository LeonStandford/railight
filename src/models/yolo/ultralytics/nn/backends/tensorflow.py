...
class TensorFlowBackend(BaseBackend):
    ...
    def __init__(
        ...
        assert format in {
            ...
            "tflite",
            "edgetpu",
        }, f"Unsupported TensorFlow format: {format}."
        self.format = format
    ...
    def load_model(self, weight: str | Path) -> None:
        ...
        if self.format == "saved_model":
            ...
            self.frozen_func = wrap_frozen_graph(
            ...
            try:
                metadata_file = next(
                    Path(weight)
                    .resolve()
                    .parent.rglob(f"{Path(weight).stem}_saved_model*/metadata.yaml")
                )
                from ultralytics.utils import YAML
            ...
            if self.format == "edgetpu":
                ...
                LOGGER.info(
                    f"Loading {weight} on device {device[1:]} for TensorFlow Lite Edge TPU inference..."
                )
                delegate = {
