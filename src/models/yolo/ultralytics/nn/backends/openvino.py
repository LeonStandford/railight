    ...
    def load_model(self, weight: str | Path) -> None:
        ...
        if isinstance(self.device, str) and self.device.startswith("intel"):
            ...
            if device_name not in core.available_devices:
                LOGGER.warning(
                    f"OpenVINO device '{device_name}' not available. Using '{fallback_device}' instead."
                )
                device_name = fallback_device
        w = Path(weight)
        if not w.is_file():
        ...
        self.inference_mode = (
            "CUMULATIVE_THROUGHPUT" if self.dynamic and self.batch > 1 else "LATENCY"
        )
        ...
        self.ov_compiled_model = core.compile_model(
            ov_model, device_name=device_name, config=config
        )
        LOGGER.info(
            f"Using OpenVINO {self.inference_mode} mode for batch={self.batch} inference on {', '.join(self.ov_compiled_model.get_property('EXECUTION_DEVICES'))}..."
        )
