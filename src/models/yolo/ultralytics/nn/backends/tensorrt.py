...
class TensorRTBackend(BaseBackend):
    ...
    def load_model(self, weight: str | Path) -> None:
        ...
        check_version(
            trt.__version__,
            "!=10.2.0",
            msg="https://github.com/ultralytics/ultralytics/pull/24367",
        )
        ...
        num = (
            range(engine.num_io_tensors)
            if self.is_trt10
            else range(engine.num_bindings)
        )
        for i in num:
            if self.is_trt10:
                ...
                profile_shape = (
                    tuple(engine.get_tensor_profile_shape(name, 0)[2])
                    if is_input
                    else None
                )
            else:
                ...
                profile_shape = (
                    tuple(engine.get_profile_shape(0, i)[1]) if is_input else None
                )
            if is_input:
                if -1 in shape:
                    self.dynamic = True
