...
class GCPRegions:
    ...
    def lowest_latency(
        ...
        if verbose:
            print(
                f"Testing GCP regions for latency (with {attempts} {('retry' if attempts == 1 else 'attempts')})..."
            )
        regions_to_test = (
            [k for (k, v) in self.regions.items() if v[0] == tier]
            if tier
            else list(self.regions.keys())
        )
        with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
            results = list(
                executor.map(lambda r: self._ping_region(r, attempts), regions_to_test)
            )
        ...
        if verbose:
            ...
            for region, mean, std, min_, max_ in sorted_results:
                ...
                if mean == float("inf"):
                    ...
                    print(
                        f"{region:<25} {location:<35} {tier:<5} {mean:.0f} ± {std:.0f} ({min_:.0f} - {max_:.0f})"
                    )
            ...
            for region, mean, std, min_, max_ in sorted_results[:top]:
                ...
                location = f"{city}, {country}"
                print(
                    f"{region} ({location}, {mean:.0f} ± {std:.0f} ms ({min_:.0f} - {max_:.0f}))"
                )
...
if __name__ == "__main__":
    regions = GCPRegions()
    top_3_latency_tier1 = regions.lowest_latency(
        top=3, verbose=True, tier=1, attempts=3
    )
