...
class BaseSolution:
    ...
    def adjust_box_label(
        self, cls: int, conf: float, track_id: int | None = None
    ) -> str | None:
    ...
    def __call__(self, *args: Any, **kwargs: Any):
        ...
        track_or_predict_speed = self.profilers[0].dt * 1000.0
        solution_speed = (self.profilers[1].dt - self.profilers[0].dt) * 1000.0
        result.speed = {
        ...
        if self.CFG["verbose"]:
            ...
            LOGGER.info(
                f"{self.frame_no}: {result.plot_im.shape[0]}x{result.plot_im.shape[1]} {solution_speed:.1f}ms, {', '.join([f'{v} {self.names[k]}' for (k, v) in counts.items()])}\nSpeed: {track_or_predict_speed:.1f}ms {track_or_predict}, {solution_speed:.1f}ms solution per image at shape (1, {getattr(self.model, 'channels', 3)}, {result.plot_im.shape[0]}, {result.plot_im.shape[1]})\n"
            )
        return result
...
class SolutionAnnotator(Annotator):
    ...
    def _estimate_pose_angle_cached(
    ...
    def draw_specific_kpts(
        ...
        points = [
            ...
            for j in indices
            if 0 <= j < n
            and (float(keypoints[j][2]) if len(keypoints[j]) > 2 else 1.0)
            >= conf_thresh
        ]
    ...
    def adaptive_label(
        ...
        text_size = cv2.getTextSize(
            label, cv2.FONT_HERSHEY_SIMPLEX, self.sf - 0.15, self.tf
        )[0]
