...
class Analytics(BaseSolution):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.total_counts = 0
        self.clswise_count = {}
        self.update_every = kwargs.get("update_every", 30)
        self.last_plot_im = None
    ...
    def process(self, im0: np.ndarray, frame_number: int) -> SolutionResults:
        ...
        if self.type == "line":
            ...
            update_required = (
                frame_number % self.update_every == 0 or self.last_plot_im is None
            )
            if update_required:
            ...
            raise ValueError(
                f"Unsupported analytics_type='{self.type}'. Supported types: line, bar, pie, area."
            )
        return SolutionResults(
            plot_im=plot_im,
            total_tracks=len(self.track_ids),
            classwise_count=self.clswise_count,
        )

    def update_graph(
        self,
        frame_number: int,
        count_dict: dict[str, int] | None = None,
        plot: str = "line",
    ) -> np.ndarray:
        if count_dict is None:
            ...
            if len(x_data) > self.max_points:
                x_data, y_data = (
                    x_data[-self.max_points :],
                    y_data[-self.max_points :],
                )
                ...
                for key in count_dict:
                    ...
                    if len(y_data_dict[key]) < max_length:
                        y_data_dict[key] = np.pad(
                            y_data_dict[key], (0, max_length - len(y_data_dict[key]))
                        )
                if len(x_data) > self.max_points:
                    x_data = x_data[1:]
                    for key in count_dict:
