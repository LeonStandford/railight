    ...
    def __call__(
        ...
        if trainer is not None:
            ...
            if refer_data is not None:
                ...
                eval_names = [
                    name.split("/", 1)[0] for name in list(eval_data["names"].values())
                ]
                if names != eval_names:
                    LOGGER.warning(
                        f"Class names from refer data {names} do not match evaluation dataset {eval_names}. This may lead to incorrect validation results."
                    )
            if load_vp:
                LOGGER.info("Validate using the visual prompt.")
                self.args.half = False
