...
class DetectionValidator(BaseValidator):
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        ...
        self.confusion_matrix = ConfusionMatrix(
            names=model.names, save_matches=self.args.plots and self.args.visualize
        )
    ...
    def postprocess(self, preds: torch.Tensor) -> list[dict[str, torch.Tensor]]:
        ...
        return [
            {"bboxes": x[:, :4], "conf": x[:, 4], "cls": x[:, 5], "extra": x[:, 6:]}
            for x in outputs
        ]
    ...
    def print_results(self) -> None:
        ...
        if self.metrics.nt_per_class.sum() == 0:
            LOGGER.warning(
                f"no labels found in {self.args.task} set, cannot compute metrics without labels"
            )
        if (
            self.args.verbose
            and (not self.training)
            and (self.nc > 1)
            and len(self.metrics.stats)
        ):
        ...
        if self.args.save_json and (self.is_coco or self.is_lvis) and len(self.jdict):
            ...
            try:
                ...
                for i, iou_type in enumerate(iou_types):
                    ...
                    stats["fitness"] = (
                        0.9 * val.stats_as_dict["AP_all"]
                        + 0.1 * val.stats_as_dict["AP_50"]
                    )
                    if self.is_lvis:
