...
def build_sam3_image_model(
    checkpoint_path: str, enable_segmentation: bool = True, compile: bool = False
):
    try:
        import clip
    except ImportError:
    ...
    vision_encoder = _create_vision_backbone(
        compile_mode=compile_mode, enable_inst_interactivity=True
    )
    ...
    segmentation_head = (
        UniversalSegmentationHead(
            ...
            cross_attend_prompt=nn.MultiheadAttention(
                num_heads=8, dropout=0, embed_dim=256
            ),
...
def build_interactive_sam3(
    checkpoint_path: str, compile=None, with_backbone=True
) -> SAM3Model:
    ...
    backbone = (
        SAM3VLBackbone(
            scalp=1, visual=_create_vision_backbone(compile_mode=compile), text=None
        )
        if with_backbone
        else None
    )
...
def _load_checkpoint(model, checkpoint, interactive=False):
    ...
    if interactive:
        ...
        sam3_image_ckpt.update(
            {k.replace("tracker.", ""): v for (k, v) in ckpt.items() if "tracker." in k}
        )
