...
class Attention(nn.Module):
    ...
    def _setup_rel_pos(
        self, rel_pos_zero_init: bool = True, input_size: tuple[int, int] | None = None
    ) -> None:
    ...
    def _setup_rope_freqs(self, input_size: tuple[int, int] | None = None) -> None:
        ...
        freqs_cis = self.compute_cis(
            end_x=input_size[0], end_y=input_size[1], scale_pos=scale_pos
        )
        if self.cls_token:
            t = torch.zeros(
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(norm_layer, str):
            norm_layer = partial(getattr(nn, norm_layer), eps=1e-05)
        ...
        if self.use_abs_pos:
            num_patches = (
                pretrain_img_size // patch_size * (pretrain_img_size // patch_size)
            )
    ...
    def set_imgsz(self, imgsz: list[int] = [1008, 1008]):
        for block in self.blocks:
            if block.window_size != 0:
                continue
            block.attn._setup_rel_pos(
                input_size=(imgsz[0] // self.patch_size, imgsz[1] // self.patch_size)
            )
