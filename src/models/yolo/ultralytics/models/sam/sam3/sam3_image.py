...
class SAM3SemanticModel(torch.nn.Module):
    ...
    def _run_decoder(
        self, pos_embed, memory, src_mask, out, prompt, prompt_mask, encoder_out
    ):
        bs = memory.shape[1]
    ...
    def _update_scores_and_boxes(
        ...
        _update_out(
            out, "pred_boxes_xyxy", outputs_boxes_xyxy[:, :, :num_o2o], update_aux=False
        )

    def _run_segmentation_heads(
        self, out, backbone_out, encoder_hidden_states, prompt, prompt_mask, hs
    ):
    ...
    def forward_grounding(
        ...
        backbone_out, img_feats, img_pos_embeds, vis_feat_sizes = (
            SAM2Model._prepare_backbone_features(
        ...
        with torch.profiler.record_function("SAM3Image._run_encoder"):
            encoder_out = self._run_encoder(
                img_feats, img_pos_embeds, vis_feat_sizes, prompt, prompt_mask
            )
