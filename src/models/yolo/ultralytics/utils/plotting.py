...
class Annotator:
    ...
    def masks(
        ...
        if im_gpu is None:
            assert isinstance(
                masks, np.ndarray
            ), "`masks` must be a np.ndarray if `im_gpu` is not provided."
            ...
            assert isinstance(
                masks, torch.Tensor
            ), "'masks' must be a torch.Tensor if 'im_gpu' is provided."
            if len(masks) == 0:
        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                ...
                for c in classes:
                    ...
                    annotator.text(
                        [x, y], label, txt_color=color, box_color=(64, 64, 64, 128)
                    )
            if len(kpts):
                kpts_ = kpts[idx].copy()
                if len(kpts_):
...
def plot_tune_results(
    results_file: str = "tune_results.ndjson", exclude_zero_fitness_points: bool = True
):
    import json
    import matplotlib.pyplot as plt
    from scipy.ndimage import gaussian_filter1d
    ...
    x = np.array(
        [
            [r.get("fitness", 0.0)]
            + [r.get("hyperparameters", {}).get(k, np.nan) for k in keys]
            for r in records
        ],
        dtype=float,
    )
    ...
    if exclude_zero_fitness_points:
        zero_mask = all_fitness > 0
        x, all_fitness = (x[zero_mask], all_fitness[zero_mask])
    if len(all_fitness) == 0:
        LOGGER.warning(
            "No valid fitness values to plot (all iterations may have failed)"
        )
        return
    ...
    for dataset in sorted({k for r in records for k in r.get("datasets", {})}):
        y = np.array(
            [
                r.get("datasets", {}).get(dataset, {}).get("fitness", np.nan)
                for r in records
            ],
            dtype=float,
        )
    ...
    plt.plot(
        x,
        gaussian_filter1d(all_fitness, sigma=3),
        ":",
        color="0.35",
        label="smoothed mean",
        linewidth=2,
