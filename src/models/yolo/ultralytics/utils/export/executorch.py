...
def executorch_wrapper(model: torch.nn.Module) -> torch.nn.Module:
    ...
    for m in model.modules():
        ...
        m.kpts_decode = types.MethodType(
            partial(_executorch_kpts_decode, is_pose26=type(m) is Pose26), m
        )
    return model


def _executorch_kpts_decode(
    self, kpts: torch.Tensor, is_pose26: bool = False
) -> torch.Tensor:
    ...
    a = (
        y[:, :, :2] + anchors if is_pose26 else y[:, :, :2] * 2.0 + (anchors - 0.5)
    ) * strides
    if ndim == 3:
...
def torch2executorch(
    ...
    et_program = to_edge_transform_and_lower(
        torch.export.export(model, (im,)), partitioner=[XnnpackPartitioner()]
    ).to_executorch()
