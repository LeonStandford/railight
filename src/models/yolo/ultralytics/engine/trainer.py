...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks: dict | None = None):
        ...
        if RANK in {-1, 0}:
            ...
            if args_dict.get("augmentations") is not None:
                args_dict["augmentations"] = [
                    repr(t) for t in args_dict["augmentations"]
                ]
    ...
    def train(self):
        if self.ddp:
            ...
            if self.args.batch < 1.0:
                raise ValueError(
                    f"AutoBatch with batch<1 not supported for Multi-GPU training, please specify a valid batch size multiple of GPU count {self.world_size}, i.e. batch={self.world_size * 8}."
                )
            cmd, file = (None, None)
            try:
    ...
    def _setup_train(self):
        ...
        if not any((v.requires_grad for v in self.model.parameters())):
            raise RuntimeError(
                f"'freeze={self.args.freeze}' froze the entire model with no trainable parameters left. Reduce 'freeze' or pass a list of specific layer indices."
            )
    ...
    def _do_train(self):
        ...
        LOGGER.info(
            f"Image sizes {self.args.imgsz} train, {self.args.imgsz} val\nUsing {self.train_loader.num_workers * (self.world_size or 1)} dataloader workers\nLogging results to {colorstr('bold', self.save_dir)}\nStarting training for "
            + (
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                try:
                    ...
                    LOGGER.warning(
                        f"CUDA out of memory with batch={old_batch}. Reducing to batch={self.batch_size} and retrying ({self._oom_retries}/3)."
                    )
    ...
    def auto_batch(self, max_num_obj=0, dataset_size=0):
        max_imgsz = int(self.args.imgsz * (1 + self.args.multi_scale))
    ...
    def read_results_csv(self):
        ...
        try:
            return pl.read_csv(self.csv, infer_schema_length=None).to_dict(
                as_series=False
            )
        except Exception:
            return {}
    ...
    def _model_train(self):
        ...
        for n, m in self.model.named_modules():
            if any(filter(lambda f: f in n, self.freeze_layer_names)) and isinstance(
                m, nn.BatchNorm2d
            ):
                m.eval()
    ...
    def save_model(self):
        ...
        if not all(
            ...
            LOGGER.warning(
                f"Skipping checkpoint save at epoch {self.epoch}: EMA contains NaN/Inf"
            )
            return False
        buffer = io.BytesIO()
        torch.save(
    ...
    def check_resume(self, overrides):
        ...
        if resume:
            try:
                ...
                if not isinstance(ckpt_args["data"], dict) and (
                    not Path(ckpt_args["data"]).exists()
                ):
                ...
                if ckpt_args.get("augmentations") is not None:
                    LOGGER.warning(
                        f"Custom Albumentations transforms were used in the original training run but are not being restored. To preserve custom augmentations when resuming, you need to pass the 'augmentations' parameter again to get expected results. Example: \nmodel.train(resume=True, augmentations={ckpt_args['augmentations']})"
                    )
            except Exception as e:
    ...
    def _handle_nan_recovery(self, epoch):
        ...
        if not self.last.exists():
            raise RuntimeError(
                f"{reason} detected but no valid last.pt is available for recovery"
            )
    ...
    def resume_training(self, ckpt):
        ...
        if getattr(unwrap_model(self.model), "end2end", False):
            unwrap_model(self.model).criterion = unwrap_model(
                self.model
            ).init_criterion()
    ...
    def build_optimizer(
        ...
        LOGGER.info(
            f"{colorstr('optimizer:')} {type(optimizer).__name__}(lr={lr}, momentum={momentum}) with parameter groups {num_params[1]} weight(decay=0.0), {num_params[0]} weight(decay={decay}), {num_params[2]} bias(decay=0.0)"
        )
        return optimizer
