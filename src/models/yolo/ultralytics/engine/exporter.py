...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if fmt == "axelera":
            if model.task == "segment" and any(
                (isinstance(m, Segment26) for m in model.modules())
            ):
                raise ValueError(
        ...
        if hasattr(model, "end2end"):
            ...
            if fmt == "engine" and self.args.int8:
                try:
                    ...
                    if check_version(trt.__version__, "==10.3.0") and is_jetson(
                        jetpack=6
                    ):
                        model.end2end = False
                        LOGGER.warning(
                            "TensorRT 10.3.0 on JetPack 6 with int8 has known end2end build issues, disabling end2end branch. For a fix, see https://docs.ultralytics.com/guides/nvidia-jetson/#why-does-my-tensorrt-int8-export-disable-end2end-on-jetpack-6"
                        )
                except ImportError:
                    pass
        ...
        if self.args.nms:
            ...
            assert (
                fmt != "tflite" or not ARM64 or (not LINUX)
            ...
            assert (
                fmt != "onnx" or TORCH_1_13
            ), "ONNX export with NMS requires torch>=1.13"
        ...
        if (
            (fmt in {"engine", "coreml"} or self.args.nms)
        ...
        if isinstance(model, WorldModel):
            LOGGER.warning(
                "YOLOWorld (original version) export is not supported to any format. YOLOWorldv2 models (i.e. 'yolov8s-worldv2.pt') only support export to (torchscript, onnx, openvino, engine, coreml) formats. See https://docs.ultralytics.com/models/yolo-world for details."
            )
        ...
        if is_tf_format:
            ...
            if fmt == "edgetpu":
                f = self.export_edgetpu(
                    tflite_model=Path(f) / f"{self.file.stem}_full_integer_quant.tflite"
                )
            if fmt == "tfjs":
                f = self.export_tfjs()
        else:
        ...
        if f:
            ...
            s = (
                ""
                if square
                else f"WARNING ⚠️ non-PyTorch val requires square images, 'imgsz={self.imgsz}' will not work. Use export 'imgsz={max(self.imgsz)}' if val is required."
            )
    ...
    def get_int8_calibration_dataloader(self, prefix=""):
        ...
        if hasattr(dataset.transforms.transforms[0], "new_shape"):
            dataset.transforms.transforms[0].new_shape = self.imgsz
        n = len(dataset)
        if n < 1:
            raise ValueError(
                f"The calibration dataset must have at least 1 image, but found {n} images."
            )
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        with arange_patch(
            dynamic=bool(dynamic), half=self.args.half, fmt=self.args.format
        ):
            torch2onnx(
    ...
    def export_paddle(self, prefix=colorstr("PaddlePaddle:")):
        ...
        return torch2paddle(
            model=self.model,
            im=self.im,
            output_dir=str(self.file).replace(
                self.file.suffix, f"_paddle_model{os.sep}"
            ),
            metadata=self.metadata,
            prefix=prefix,
        )
    ...
    def export_pb(self, keras_model, prefix=colorstr("TensorFlow GraphDef:")):
        ...
        return keras2pb(
            keras_model, output_file=self.file.with_suffix(".pb"), prefix=prefix
        )
    ...
    def export_imx(self, prefix=colorstr("IMX:")):
        assert (
            LINUX
        ), "Export only supported on Linux.See https://developer.aitrios.sony-semicon.com/en/docs/raspberry-pi-ai-camera/imx500-converter?version=3.17.3&progLang="
        assert (
            IS_PYTHON_MINIMUM_3_9
