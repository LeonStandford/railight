...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self) -> list[dict]:
        ...
        if not labels:
            ...
            raise RuntimeError(
                f"No valid images found in {cache_path}.\n  {issues}\n{HELP_URL}"
            )
        ...
        if len_segments and len_boxes != len_segments:
            LOGGER.warning(
                f"Box and segment counts should be equal, but got len(segments) = {len_segments}, len(boxes) = {len_boxes}. To resolve this only boxes will be used and all segments will be removed. To avoid this please supply either a detect or segment dataset, not a detect-segment mixed dataset."
            )
            for lb in labels:
                lb["segments"] = []
        if len_cls == 0:
...
class ClassificationDataset:
    ...
    def __init__(self, root: str, args, augment: bool = False, prefix: str = ""):
        ...
        if self.cache_ram:
            LOGGER.warning(
                "Classification `cache_ram` training has known memory leak in https://github.com/ultralytics/ultralytics/issues/9824, setting `cache_ram=False`."
            )
