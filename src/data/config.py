...
def apply_input_config(args_ns):
    ...
    if hasattr(args_ns, "flip_label_swap"):
        # An explicit null/[] means "no swap on horizontal flip" -- the pairs
        # below are left-/right-rail classes, so a config without them (fewer
        # classes, or a class list where flipping changes nothing) has to be
        # able to switch the remap off rather than fall back to the default.
        swap = args_ns.flip_label_swap or []
