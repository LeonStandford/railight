"""Train YOLO26<scale> on WIDER Face using the in-repo ultralytics package."""
...
_YOLO_ROOT = os.path.join(_HERE, 'yolo')
...
class YoloDataset(data.Dataset):
    """Generic YOLO-format detection dataset.

    Expected layout::

        <root>/
            images/<name>.{jpg,png,...}
            labels/<name>.txt   # each line: cls cx cy w h  (xywh normalized)

    Args:
        images_dir:   folder containing input images.
        labels_dir:   folder containing per-image .txt label files.
        imgsz:        target square size (resize to imgsz x imgsz).
        max_class:    keep boxes with cls < max_class (clips classes >=
                      max_class — useful when the label files were produced
                      with more classes than the current head supports).
    """
...
def yolo_collate(batch: List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]],
                 ) -> Batch:
    ...
    for i, (img, cls, bxywh) in enumerate(batch):
        ...
        batch_idx.append(
            torch.full((cls.shape[0],), i, dtype=torch.float32),
        )
...
def build_yolo26(model_yaml: str, scale: str, nc: int,
                 imgsz: int, device: torch.device) -> DetectionModel:
    ...
    model.args = get_cfg(overrides=dict(
        box=7.5, cls=0.5, dfl=1.5, imgsz=imgsz,
    ))
    return model
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--scale', type=str, default='n',
                   choices=['n', 's', 'm', 'l', 'x'])
    p.add_argument('--imgsz', type=int, default=640)
    p.add_argument('--nc', type=int, default=3,
                   help='Number of classes (must match the detection head)')
    p.add_argument('--data_root', type=str, required=True,
                   help='Root with <split>/images and <split>/labels '
                        '(e.g. .../source). Splits expected: Train, Val.')
    p.add_argument('--train_split', type=str, default='Train')
    p.add_argument('--val_split', type=str, default='Val')
    p.add_argument('--max_class', type=int, default=3,
...
def build_loader(dataset: data.Dataset, batch_size: int, num_workers: int,
                 dctx: DistContext) -> data.DataLoader:
    if dctx.enabled:
...
def warmup_lr(optimizer: optim.Optimizer, base_lr: float,
...
def save_ckpt(model: torch.nn.Module, path: str, args: argparse.Namespace,
              epoch: int) -> None:
    torch.save(
        {'epoch': epoch, 'weight': model.state_dict(), 'args': vars(args)},
        path,
    )
...
def main() -> None:
    ...
    if dctx.enabled:
        model = DDP(model, device_ids=[dctx.local_rank],
    ...
    train_ds = YoloDataset(
        ...
        imgsz=args.imgsz, max_class=args.max_class,
    )
    if dctx.rank0:
        print(f'Train: {len(train_ds)} images from {train_images}')
    train_loader = build_loader(
        train_ds, args.batch_size, args.num_workers, dctx,
    )

    optimizer = optim.SGD(
        [p for p in model.parameters() if p.requires_grad],
        lr=args.lr0, momentum=args.momentum,
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            if dctx.rank0 and global_iter % args.print_every == 0:
                ...
                print(
                    f'epoch {epoch:3d}/{args.epochs:d} | '
                    f'iter {global_iter:6d} | lr {lr_now:.4g} | '
                    f'loss {scalar(loss):.4f} | box {box:.4f} | '
                    f'cls {cls:.4f} | dfl {dfl:.4f} | dt {time.time()-t0:.3f}s'
                )
            global_iter += 1
        ...
        if dctx.rank0:
            ...
            print(
                f'[epoch {epoch:3d}] mean_loss {mean_loss:.4f} | '
                f'box {sum_box/n:.4f} | cls {sum_cls/n:.4f} | '
                f'dfl {sum_dfl/n:.4f} | time {time.time()-t_epoch:.1f}s'
            )

            if (epoch + 1) % args.save_every == 0:
                inner_model = model.module if dctx.enabled else model
                save_ckpt(inner_model, os.path.join(save_dir, 'last.pth'),
                          args, epoch)
                if mean_loss < best_loss:
                    best_loss = mean_loss
                    save_ckpt(inner_model,
                              os.path.join(save_dir, 'best.pth'),
                              args, epoch)
                    print(
                        f'  -> new best (mean_loss={best_loss:.4f}), '
                        f'saved best.pth'
                    )
